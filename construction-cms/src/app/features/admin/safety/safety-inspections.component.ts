...
import { SafetyInspection } from '../../../shared/interfaces';
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div>
            <h1 class="text-3xl font-black text-slate-900 dark:text-white mb-2 tracking-tight uppercase tracking-tight">
              {{ 'admin.safety_inspections' | translate }}
            </h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">
              {{ 'admin.safety_inspections_desc' | translate }}
            </p>
          </div>
          ...
          <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-white/5 shadow-xl p-6 mb-8">
            <div class="flex flex-col md:flex-row gap-6">
              ...
              <div>
                ...
                >
                  <option value="">{{ 'admin.all' | translate }}</option>
                  <option value="Routine">{{ 'admin.routine' | translate }}</option>
                  <option value="Incident">{{ 'admin.incident' | translate }}</option>
                  <option value="Compliance">{{ 'admin.compliance' | translate }}</option>
                </select>
              </div>
            </div>
          </div>
...
export class SafetyInspectionsComponent implements OnInit {
    ...
    loadInspections() {
        this.isLoading = true;
        this.safetyService.getInspections().subscribe({
            next: (data: any) => {
    ...
    createInspection() {
        ...
        this.safetyService.createInspection(this.newInspection).subscribe({
            next: () => {
    ...
    startInspection(inspection: SafetyInspection) {
        console.log('Start inspection:', inspection);
    }

    completeInspection(inspection: SafetyInspection) {
        console.log('Complete inspection:', inspection);
    }

    viewDetails(inspection: SafetyInspection) {
