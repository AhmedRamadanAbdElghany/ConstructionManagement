            ...
            <div class="flex flex-wrap gap-2 mt-1.5">
               @for (item of node.items; track item.id) {
                  <div class="relative group/item">
                     <button (click)="onEditItem.emit({item, phase: node})" class="flex flex-col text-left space-y-0.5 bg-emerald-500/5 dark:bg-emerald-500/10 p-2 rounded-xl border border-emerald-500/10 hover:border-emerald-500/40 hover:bg-emerald-500/10 transition-all group/item-btn">
                        <span class="text-[9px] font-black text-emerald-600 uppercase tracking-widest group-hover/item-btn:text-emerald-700">{{ item.name || item.description }}</span>
