    ...
    <div [class.w-72]="!isCollapsed()" [class.w-24]="isCollapsed()" 
      ...
      <nav class="flex-1 p-4 space-y-2 overflow-y-auto custom-scrollbar pt-8">
        ...
        @if (isAdmin) {
          ...
          <!-- Access Control -->
          <div class="my-4 px-4 h-px bg-slate-100 dark:bg-slate-800/60"></div>

          @if (currentRole !== 'SuperAdmin') {
            <a routerLink="/admin/access-control/roles" 
               routerLinkActive="nav-active"
               class="nav-item group">
              <div class="nav-icon-box">
                <svg class="w-5 h-5 transition-transform group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <span class="nav-label" [class.opacity-0]="isCollapsed()" [class.w-0]="isCollapsed()">Manage Roles</span>
            </a>
          }
        }
